...
def unstack(input_tensor):
    input_ = tf.expand_dims(input_tensor, axis=2)  # b x M x 1 x d
    return tf.unstack(input_, input_.shape[1], 1)  # [b x 1 x d] * M
...
def FGCNN(dnn_feature_columns, embedding_size=8, conv_kernel_width=(7, 7, 7, 7), conv_filters=(14, 16, 18, 20),
    ...
    if len(conv_filters) > 0:
        ...
        combined_input = origin_input # batch x M x embedding size
